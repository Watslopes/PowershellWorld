...
function Get-LWAPIToken2{	 
    ...
    $response = Invoke-RestMethod -Uri 'https://sit-node1.if-Application.corporate.company.com:81/api/authentication/login' -Method 'GET' -Headers $headers #-ContentType "application/x-www-form-urlencoded"	
    Return $response
}
...
$eventHandler = [System.Windows.RoutedEventHandler]{
        try{    
            ...
            {$RestRqst = Invoke-WebRequest -Uri $gui_txtbox_apiurl.Text -Method $gui_combo_apitype.Text -ContentType $contenttype -Headers @{"Authorization"="Basic $getLWToken"}} 
            Else 
            {$body = $gui_txtbox_payload.Text; $RestRqst = Invoke-WebRequest -Uri $gui_txtbox_apiurl.Text -Method $gui_combo_apitype.Text -Body $body -ContentType $contenttype -Headers @{"Authorization"="Basic $getLWToken"}}
...
Invoke-RestMethod -Uri "https://sit-node1.if-Application.corporate.company.com:81/api/authentication/logoff" -Method GET -Headers $headers 